-----------------------------------------------------------------------------
-- Test script
-- Scenario 10.1
-----------------------------------------------------------------------------

local cen = "10.5"
suc_msg = "Scenario " .. cen .. ": ok!"
err_msg = "Scenario " .. cen .. ": erro!"
local daemonlist = {"127.0.0.1:8888/0", "127.0.0.1:8889/0", "127.0.0.1:8890/0", "127.0.0.1:8891/0", "127.0.0.1:8892/0"}
local count = 0

function storecb(reply)
    local ret = assert(reply.status == alua.ALUA_STATUS_OK, err_msg)
    count = count + 1
    if count == 4 then
        print(suc_msg)
        for i,v in ipairs(daemonlist) do
            alua.send(v, "alua.quit()")
            alua.quit()
        end
    end
end

function main()
    alua.dht_insert("number", 123456, storecb)
    alua.dht_insert("string", "123456", storecb)
    alua.dht_insert("boolean", true, storecb)
    alua.dht_insert("table", {123456, "123456", true}, storecb)
end
